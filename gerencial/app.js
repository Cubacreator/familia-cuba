...
let DATA={lavagens:[],gastos:[],metas:[],acoes:[],clientes:[],membros:[],auditoria:[],config:{pct_maquina:57}},profile=null,current="dashboard";
...
async function loadAll(){
 const tables=["lavagens","gastos","metas","acoes","clientes","membros"];
...
function openModal(id,rec){
 ...
 if(id==="gastoModal"&&!rec)f.elements.data.value=today();if(id==="acaoModal"&&!rec)f.elements.data.value=today();if(id==="metaModal")updateMetaExempt()
}
...
function editRecord(type,id){const map={lavagens:["lavModal","lavagens"],gastos:["gastoModal","gastos"],metas:["metaModal","metas"],acoes:["acaoModal","acoes"],clientes:["clienteModal","clientes"],membros:["membroModal","membros"]};const [modal,t]=map[type],r=DATA[t].find(x=>x.id===id);openModal(modal,r)}window.editRecord=editRecord;
...
function renderDashboard(){
 ...
 const rank=rankingHTML(L);$("#top5").innerHTML=rank.length?rank.slice(0,5).map((r,i)=>`<div class="rank"><i>${i+1}</i><div>${r.nome}<small>${r.qtd} lavagem${r.qtd===1?"":"ens"}</small></div><b>${money(r.sujo)}</b></div>`).join(""):`<div class="empty">Sem lavagens no período.</div>`;
...
function renderAll(){renderLists();renderDashboard();renderLavagens();renderGastos();renderMetas();renderAcoes();renderClients();renderMembers();renderAudit()}
