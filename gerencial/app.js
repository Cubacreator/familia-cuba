...
function actionFamilyShare(x){
  if(x.status==="Falhou")return 0;
  if(x.valor_familia!=null)return n(x.valor_familia);
  if(x.entrada_familia===true)return n(x.valor_ganho)||n(x.entrada);
  return n(x.entrada);
}
...
function renderDashboard(){
 ...
 $("#kSujo").textContent=money(sum(L,"valor_sujo"));$("#kLimpo").textContent=money(sum(L,"valor_limpo"));$("#kCliente").textContent=money(sum(L,"valor_cliente"));$("#kCuba").textContent=money(sum(L,"valor_cuba")+sum(LD,"valor_liquido_real")+actionContributionDetails(A).total-sum(A,"custo")-sum(GR,"valor")-sum(M,"valor_pagamento_limpo"));$("#kCubaLavagens").textContent=money(sum(L,"valor_cuba"));$("#kInsumos").textContent=money(sum(L,"custo_insumos"));$("#kGastos").textContent=money(sum(GR,"valor")+sum(A,"custo"));const ac=actionContributionDetails(A);$("#kAcoes").textContent=money(ac.totalReal);$("#kFixosSujo").textContent=money(FP.filter(x=>x.tipo_dinheiro==="sujo").reduce((s,x)=>s+n(x.valor),0));$("#kDolLimpo").textContent=money(sum(LD,"valor_liquido_real"));$("#kGastosDol").textContent=usd(sum(GD,"valor"));
