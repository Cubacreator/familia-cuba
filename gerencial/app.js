...
function renderDashboard(){
 ...
 $("#kSujo").textContent=money(sum(L,"valor_sujo"));$("#kLimpo").textContent=money(sum(L,"valor_limpo"));$("#kCliente").textContent=money(sum(L,"valor_cliente"));$("#kCuba").textContent=money(sum(L,"valor_cuba")+sum(LD,"valor_liquido_real")+sum(A,"entrada")-sum(A,"custo")-sum(GR,"valor")-sum(M,"valor_pagamento_limpo"));$("#kGastos").textContent=money(sum(G,"valor")+sum(A,"custo"));$("#kAcoes").textContent=money(sum(A,"entrada"));$("#kFixosSujo").textContent=money(FP.filter(x=>x.tipo_dinheiro==="sujo").reduce((s,x)=>s+n(x.valor),0));$("#kDolLimpo").textContent=money(sum(LD,"valor_liquido_real"));
