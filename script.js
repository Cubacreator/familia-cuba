...
let i=0;const load=setInterval(()=>{loaderText.textContent=mensagens[i];bar.style.width=((i+1)/mensagens.length*100)+'%';i++;if(i===mensagens.length){clearInterval(load);setTimeout(()=>{loader.style.opacity='0';loader.style.transition='.8s';setTimeout(()=>{loader.remove();site.classList.remove('hidden');observe()},800)},700)}},1.800);
